...
def inverse_kinematics(current_q, target_dir, target_pos):
    urdf_filename = '/home/wqs/gen3_rl_5_26/gen3_model/gen3_description/robots/gen3_robotiq_2f_85.urdf'
    # 从 URDF 文件构建机器人模型
    ...
    nq = model.nq
        ...
        # 创建完整的配置向量（17个），前7个用current_q，其余设为0
    # q = np.zeros(nq)
    # q[:7] = current_q
    ...
    oMdes = pinocchio.SE3(np.eye(3), np.array(target_pos))
    ...
    while True:
        ...
        current_pos = data.oMi[JOINT_ID].translation
        pos_err = oMdes.translation - current_pos
        # 计算目标位姿到当前位姿之间的变换
        ...
        J_full = pinocchio.computeJointJacobian(model, data, q, JOINT_ID)
        J = J_full[:3, :]  # 只用位置雅可比

        dq = solve(J @ J.T + damp * np.eye(3), pos_err)
        q = pinocchio.integrate(model, q, J.T @ dq * DT)
...
def main():
    ...
    #     if x < 0.8: 
    #         x += 0.004
    #         new_q = inverse_kinematics(initial_q, R_x, [x, 0.4, 0.4]) 
    #     # data.qpos[:7] = new_q
    #     initial_q = new_q[:7]
    #     data.qpos[:7] = new_q[:7]
    ...
    # 圆轨迹参数
    center = np.array([0.6, 0.4, 0.4])  # 圆心
    radius = 0.1                       # 半径
    angle = 0                         # 初始角度
    d_angle = 0.02                   # 每帧角度增量
    ...
    while not glfw.window_should_close(window):
        ...
        # 求解逆运动学，目标位置沿圆轨迹移动，姿态固定
        new_q = inverse_kinematics(initial_q, R_x, [target_x, target_y, target_z])
